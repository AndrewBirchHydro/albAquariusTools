...
###
###Most of this first section was adapted from Dean Tucker's (NPS-WRD) tools for accessing Aquarius TS data
###
...
#####
#####
##### Now loading ALB analysis and visualization tools


###
### function "TS_simplify" takes a time series of a given water quality parameter and reforms the date and time into a usable format
###

TS_simplify <- function(data){
  require(dplyr)
  require(tidyr)
  TSdata <- data$Points
  test <- as.data.frame(TSdata[,c(1,2)])
  test$rawdate <- test$Timestamp
  date_times2 <-  separate(data = test, col = rawdate, into = c("left", "right"), sep = "T")
  date_times2$value <- date_times2$NumericValue1
  date_times2$date <- date_times2$left
  date_times2$int <- date_times2$right
  date_times3 <- date_times2[,c("value","date","int")]
  date_times4 <-  separate(data = date_times3, col = int, into = c("time", "junk"), sep = "\\.")
  date_times4$date_time <- paste(date_times4$date,date_times4$time,sep=" ")
  date_times4$date_time <- as.POSIXct(date_times4$date_time, format = "%Y-%m-%d %H:%M:%S")
  output <- date_times4[, c("date_time","value")]
  return(output)}
...
###
###Function "TS_plot1" uses "TS_simplify" to simlify data, then plots a time series at the original resolution
###


TS_plot1 <- function(data){
  require(ggplot2)
  tsdata <- data
  Location_name <- tsdata$TimeSeries$LocationIdentifier
  parameter <- tsdata$TimeSeries$Parameter
  simplified <- TS_simplify(data=data)
  plot<- ggplot(simplified, aes(x=date_time, y=value)) + geom_path() + theme_bw() + ggtitle(paste(Location_name,",","Original Resolution")) + ylab(parameter)
  return(plot)}



###
###Function "TS_monthly" simplifies the raw original tsdata and aggregates it to monthly mean, minimum, maximum and standard deviation. It also ###adds numeric factor values for the month and year individually
###

TS_monthly <- function(data){
  require(lubridate)
  raw_simp <- TS_simplify(data=data)
  mon<- aggregate(raw_simp["value"],
                  list(day = cut(raw_simp$date, breaks="month")), mean)
  mon$month <- as.Date(mon$day, "%Y-%m-%d")
  mon$month_num <- as.factor(month(mon$month))
  mon$year <- as.factor(year(mon$month))
  mon$Mean <- mon$value
  mon_exp <- mon[,c("month","month_num","year","Mean")]
  StDev <- aggregate(raw_simp["value"],
                     list(day = cut(raw_simp$date, breaks="month")), sd)
  StDev$StDev <- StDev$value
  StDev <- StDev[,"StDev"]
  Min <- aggregate(raw_simp["value"],
                   list(day = cut(raw_simp$date, breaks="month")), min)
  Min$Min <- Min$value
  Min <- Min[,"Min"]
  Max <- aggregate(raw_simp["value"],
                   list(day = cut(raw_simp$date, breaks="month")), max)
  Max$Max <- Max$value
  Max <- Max[,"Max"]
  mon_exp2 <- cbind(mon_exp, Min, Max, StDev)
  return(mon_exp2)}



###
###Function "TS_plot2" plots the mean value of a paramter on a monthly basis. Gray lines represent monthly mean and max
###

TS_plot2 <- function(data){
  require(ggplot2)
  tsdata <- data
  Location_name <- tsdata$TimeSeries$LocationIdentifier
  parameter <- tsdata$TimeSeries$Parameter
  monthly <- TS_monthly(data=data)
  plot<- ggplot(monthly, aes(x=month, y=Mean)) + geom_path() + geom_point(size=3, aes(color=year))+ theme_bw() + ggtitle(paste(Location_name,",","Monthly Resolution (means)")) + geom_path(color="gray", aes(x=month, y=Min)) + geom_path(color="gray", aes(x=month, y=Max))+ ylab(parameter)
  return(plot)}



###
###Function "TS_plot3" plots the mean value of a parameter in each year for comparison
###

TS_plot3 <- function(data){
  require(ggplot2)
  tsdata <- data
  Location_name <- tsdata$TimeSeries$LocationIdentifier
  parameter <- tsdata$TimeSeries$Parameter
  monthly <- TS_monthly(data=data)
  plot <- ggplot(monthly, aes(x=month_num, y=Mean, color=year, group=year)) + geom_path() + geom_point() + theme_bw() + ggtitle(paste(Location_name,",","Monthly Inter-annual Comparison (means)"))+ ylab(parameter)
  return(plot)}



###
###Function "TS_plot4" plots months individually by year. Requires input of the month of interest
###

TS_plot4 <- function(data, month_enter){
  require(ggplot2)
  tsdata <- data
  Location_name <- tsdata$TimeSeries$LocationIdentifier
  parameter <- tsdata$TimeSeries$Parameter
  monthly <- TS_monthly(data=data)
  monthly$month_num <- as.numeric(as.character(monthly$month_num))
  monthly$year <- as.numeric(as.character(monthly$year))
  monthly2 <- subset(monthly, month_num == month_enter)
  plot <- ggplot(monthly2, aes(x=year, y=Mean)) + geom_path() + geom_point(size=2) + theme_bw() + ggtitle(paste(Location_name,",",month_enter)) + geom_smooth(method=lm, se=F)+ ylab(parameter)
  return(plot)}



###
###Function "ALBPlot_monthly_grid" plots all monthly trends for a given range of years in the dataset
###

TS_plot5 <- function(data){
  require(ggplot2)
  require(gridExtra)
  tsdata <- data
  Location_name <- tsdata$TimeSeries$LocationIdentifier
  parameter <- tsdata$TimeSeries$Parameter
  p1 <- TS_plot4(data=data, month_enter =1) + theme(text = element_text(size = 7)) + theme(axis.title.x=element_blank())
  p2 <- TS_plot4(data=data, month_enter =2)+ theme(text = element_text(size = 7)) + theme(axis.title.x=element_blank())
  p3 <- TS_plot4(data=data, month_enter =3)+ theme(text = element_text(size = 7)) + theme(axis.title.x=element_blank())
  p4 <- TS_plot4(data=data, month_enter =4)+ theme(text = element_text(size = 7)) + theme(axis.title.x=element_blank())
  p5 <- TS_plot4(data=data, month_enter =5)+ theme(text = element_text(size = 7)) + theme(axis.title.x=element_blank())
  p6 <- TS_plot4(data=data, month_enter =6)+ theme(text = element_text(size = 7)) + theme(axis.title.x=element_blank())
  p7 <- TS_plot4(data=data, month_enter =7)+ theme(text = element_text(size = 7)) + theme(axis.title.x=element_blank())
  p8 <- TS_plot4(data=data, month_enter =8)+ theme(text = element_text(size = 7)) + theme(axis.title.x=element_blank())
  p9 <- TS_plot4(data=data, month_enter =9)+ theme(text = element_text(size = 7)) + theme(axis.title.x=element_blank())
  p10 <- TS_plot4(data=data, month_enter =10)+ theme(text = element_text(size = 7)) + theme(axis.title.x=element_blank())
  p11 <- TS_plot4(data=data, month_enter =11)+ theme(text = element_text(size = 7)) + theme(axis.title.x=element_blank())
  p12 <- TS_plot4(data=data, month_enter =12)+ theme(text = element_text(size = 7)) + theme(axis.title.x=element_blank())
  plot <- grid.arrange(p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12)
  return(plot)}

###
###TS_plot6 Function plots a colored grid of annual parameter dynamics
###


TS_plot6 <- function(data){
  require(ggplot2)
  tsdata <- data
  Location_name <- tsdata$TimeSeries$LocationIdentifier
  parameter <- tsdata$TimeSeries$Parameter
  mtly <- TS_monthly(data=data)
  plot <- ggplot(mtly, aes(x=month_num, y= year, color=Mean)) + geom_point(size=15, shape=15) 
  return(plot)}

TS_plot7 <- function(data){
  require(lubridate)
  require(ggplot2)
  require(plotly)
  parameter <- data$TimeSeries$Parameter
  Location_name <- data$TimeSeries$LocationIdentifier
  datax <- TS_simplify(data)
  avg<- aggregate(datax["value"],
                  list(day = cut(datax$date_time, breaks="year")), mean)
  avg$year <- as.Date(avg$day, "%Y-%m-%d")
  avg$avg <- avg$value
  min<- aggregate(datax["value"],
                  list(day = cut(datax$date_time, breaks="year")), min)
  min$year <- as.Date(min$day, "%Y-%m-%d")
  min$min <- min$value
  max<- aggregate(datax["value"],
                  list(day = cut(datax$date_time, breaks="year")), max)
  max$year <- as.Date(max$day, "%Y-%m-%d")
  max$max <- max$value
  out1 <- merge(x= min, y= max, by= 'year', all.x= F, all.y = F)
  out2 <- merge(x= out1, y= avg, by= 'year', all.x= F, all.y = F)
  out3 <- out2[,c("year","avg","min","max")]
  out3$Year <- as.factor(year(out3$year))
  out4 <- out3
  plot_avg <- ggplot(out3, aes(x=year, y= avg)) + theme_bw() + geom_point(aes(text = paste(Year,"mean =",avg))) + geom_path(data=out4, aes(x=year, y=avg)) + ggtitle(paste(Location_name,"Annual Mean"," ",parameter))
  plot <- ggplotly(plot_avg, tooltip = "text")
  return(plot)}


TS_plot8 <- function(data){
  require(lubridate)
  require(ggplot2)
  require(plotly)
  require(gridExtra)
  ...
  out3$Year <- as.factor(year(out3$year))
  out4 <- out3
  plot_min <- ggplot(out3, aes(x=year, y= min)) + theme_bw() + geom_point(color="gray") + geom_path(color="gray",data=out4, aes(x=year, y=min)) + ggtitle(paste(Location_name,"Annual Minimum"," ",parameter))
  plot_avg <- ggplot(out3, aes(x=year, y= avg)) + theme_bw() + geom_point(color="blue") + geom_path(color="blue",data=out4, aes(x=year, y=avg)) + ggtitle(paste(Location_name,"Annual Mean"," ",parameter))
  plot_max <- ggplot(out3, aes(x=year, y= max)) + theme_bw() + geom_point(color="red") + geom_path(color="red",data=out4, aes(x=year, y=max)) + ggtitle(paste(Location_name,"Annual Maximum"," ",parameter))
  oro <- grid.arrange(plot_max, plot_avg, plot_min)
  return(oro)}
...
######
######
######Data Retrieval and indexing tools
...
Print_datasets <- function(Location){
  getlocds <- GET(paste0(publishapiurl,'/GetTimeSeriesDescriptionList?LocationIdentifier=',Location,sep=''))
...
BULK_simplifyALL <- function(parameter){
  require(dplyr)
  require(tidyr)
  require(lubridate)
  TW_timeseriesdescriptions=timeseries$getTimeSeriesDescriptions(locationIdentifier = "",
                                                                 parameter = parameter,
                                                                 publish = "",
  ...
  test <-TW_timeseriesdescriptions
  test$rawstart <- test$CorrectedStartTime
  test$rawend <- test$CorrectedEndTime
  date_timesS2 <-  separate(data = test, col = rawstart, into = c("start_date", "timeS"), sep = "T")
  date_timesS3 <- separate(data = date_timesS2, col = rawend, into = c("end_date", "timeE"), sep = "T")
  date_timesS4 <-  separate(data = date_timesS3, col = timeS, into = c("start_time", "junk1"), sep = "\\.")
  date_timesS5 <-  separate(data = date_timesS4, col = timeE, into = c("end_time", "junk2"), sep = "\\.")
  date_timesS5$Record_Start <- paste(date_timesS5$start_date,date_timesS5$start_time, sep=" ")
  date_timesS5$Record_End <- paste(date_timesS5$end_date,date_timesS5$end_time, sep=" ")
  date_timesS5$Record_Start <- as.POSIXct(date_timesS5$Record_Start, format = "%Y-%m-%d %H:%M:%S")
  date_timesS5$Record_End <- as.POSIXct(date_timesS5$Record_End, format = "%Y-%m-%d %H:%M:%S")
  date_timesS5$yearS <- year(date_timesS5$Record_Start)
  date_timesS5$yearE <- year(date_timesS5$Record_End)
  date_timesS5$Length_of_Obs <- date_timesS5$yearE - date_timesS5$yearS
  output <- date_timesS5[,c("Identifier","LocationIdentifier","Parameter","Unit","TimeSeriesType","Label", "Publish","Record_Start", "Record_End","Length_of_Obs")] 
  return(output)}
...
MAP_locations2 <- function(){
  require(maps)
  require(ggplot2)
  world_map <- map_data("world")
  base_world_messy <- p + geom_polygon(data=world_map, aes(x=long, y=lat, group=group), 
                                       colour="light gray", fill="light gray")
  out <- base_world_messy + theme_bw() + geom_point(data= Location_LatLong, aes(x=Long, y=Lat)) 
  return(out)}




MAP_locations_par <- function(parameter, Minimum_length){
  LatLong <- read.csv("https://raw.githubusercontent.com/AndrewBirchHydro/albAquariusTools/main/Location%20latlong.csv")
  Record_list <- BULK_simplifyALL(parameter = parameter)
  Record_list$record <- Record_list$Identifier
  Record_list$Identifier <- Record_list$LocationIdentifier
  R2 <- Record_list[,c("Identifier","record", "Record_Start", "Record_End", "Length_of_Obs")]
  R3 <- merge(x= R2, y= LatLong, by= 'Identifier', all.x= F, all.y = F)
  R3 <- subset(R3, Length_of_Obs >= Minimum_length)
  require(maps)
  require(ggplot2)
  require(plotly)
  require(rgdal)
  ...
  require(broom)
  world_map <- map_data("world")
  world_map$dum <- 1:nrow(world_map)
  R3$dum <-99339:(nrow(R3)+99338)
  mess <- merge(x= world_map, y= R3, by= 'dum', all.x= T, all.y = T)
  out <- ggplot(mess, aes(x=Long, y= Lat, text=paste("Name: ",record,"<br>Length: ", Length_of_Obs, "yrs","<br>Start: ", Record_Start, "<br>End: ", Record_End))) + geom_polygon(aes(x=long, y=lat, group=group), colour="light gray", fill="light gray") + geom_point() + xlab("Longitude") + ylab("Latitude") + ggtitle(paste(parameter,",", nrow(R3)," sites with >/= ",Minimum_length,"yrs of data"))+ geom_point() + coord_fixed()
  out3 <- ggplotly(out, dynamicTicks = T, tooltip = "text")
  return(out3)}


MAP_locations_list <- function(list, name){
  LatLong <- Location_LatLong
  R2 <- list
  R2$Identifier <- R2$Location
  R3 <- merge(x= R2, y= LatLong, by= 'Identifier', all.x= F, all.y = F)
  require(maps)
  require(ggplot2)
  require(plotly)
  ...
  R3$dum <-99339:(nrow(R3)+99338)
  mess <- merge(x= world_map, y= R3, by= 'dum', all.x= T, all.y = T)
  out <- ggplot(mess, aes(x=Long, y= Lat, text=paste("Name: ",Location))) + geom_polygon(aes(x=long, y=lat, group=group), colour="light gray", fill="light gray") + geom_point() + xlab("Longitude") + ylab("Latitude") + ggtitle(name)+ geom_point() + coord_fixed()
  out3 <- ggplotly(out, dynamicTicks = T, tooltip = "text")
  return(out3)}

MAP_locations_listv2 <- function(list, name){
  LatLong <- Location_LatLong
  R2 <- list
  R2$Identifier <- R2$LocationIdentifier
  R3 <- merge(x= R2, y= LatLong, by= 'Identifier', all.x= F, all.y = F)
  require(maps)
  require(ggplot2)
  require(plotly)
  ...
  R3$dum <-99339:(nrow(R3)+99338)
  mess <- merge(x= world_map, y= R3, by= 'dum', all.x= T, all.y = T)
  out <- ggplot(mess, aes(x=Long, y= Lat, text=paste("Name: ",LocationIdentifier))) + geom_polygon(aes(x=long, y=lat, group=group), colour="light gray", fill="light gray") + geom_point() + xlab("Longitude") + ylab("Latitude") + ggtitle(name)+ geom_point() + coord_fixed()
  out3 <- ggplotly(out, dynamicTicks = T, tooltip = "text")
  return(out3)}

MAP_locations_2lists <- function(list1, list2, name){
  LatLong <- Location_LatLong
  R2 <- list1
  R2$Identifier <- R2$Location
  R3 <- merge(x= R2, y= LatLong, by= 'Identifier', all.x= F, all.y = F)
  S2 <- list2
  S2$Identifier <- S2$Location
  S3 <- merge(x= S2, y= LatLong, by= 'Identifier', all.x= F, all.y = F)
  require(maps)
  require(ggplot2)
  require(plotly)
  require(rgdal)
  ...
  R3$dum <-99339:(nrow(R3)+99338)
  mess <- merge(x= world_map, y= R3, by= 'dum', all.x= T, all.y = T)
  out <- ggplot(mess, aes(x=Long, y= Lat, text=paste("Name: ",LocationIdentifier))) + geom_polygon(aes(x=long, y=lat, group=group), colour="light gray", fill="light gray") + geom_point(shape=4, color="red", data=S3, aes(x=Long, y=Lat))  + geom_point(color="blue") + xlab("Longitude") + ylab("Latitude") + ggtitle(name)+ geom_point() + coord_fixed()
  out3 <- ggplotly(out, dynamicTicks = T, tooltip = "text")
  return(out3)}

MAP_NWIS <- function(NWIS_list, name){
  require(maps)
  require(ggplot2)
  require(plotly)
  require(rgdal)
  ...
  world_map <- map_data("world")
  world_map$dum <- 1:nrow(world_map)
  NWIS_list$dum <-99339:(nrow(NWIS_list)+99338)
  mess <- merge(x= world_map, y= NWIS_list, by= 'dum', all.x= T, all.y = T)
  out <- ggplot(mess, aes(x=dec_long_va, y= dec_lat_va, text=paste("Name: ",station_nm,"<br>ID: ", site_no, "<br>Length:", peak_count_nu))) + geom_polygon(aes(x=long, y=lat, group=group), colour="light gray", fill="light gray") + geom_point() + xlab("Longitude") + ylab("Latitude") + ggtitle(name)+ geom_point() + coord_fixed()
  out3 <- ggplotly(out, dynamicTicks = T, tooltip = "text")
  return(out3)}

MAP_NWIS_par <- function(NWIS_list, Record_list, name){
  require(maps)
  require(ggplot2)
  require(plotly)
  require(rgdal)
  ...
  NWIS_list$dum <-99339:(nrow(NWIS_list)+99338)
  mess <- merge(x= world_map, y= NWIS_list, by= 'dum', all.x= T, all.y = T)
  out <- ggplot(mess, aes(x=dec_long_va, y= dec_lat_va, text=paste("Name: ",station_nm,"<br>ID: ", site_no, "<br>Length:", peak_count_nu))) + geom_polygon(aes(x=long, y=lat, group=group), colour="light gray", fill="light gray") + geom_point() + xlab("Longitude") + ylab("Latitude") + ggtitle(name)+ geom_point() + coord_fixed() + geom_point(shape=4, color="red",data=Record_list, aes(x=Long, y=Lat, text=loc))
  out3 <- ggplotly(out, dynamicTicks = T, tooltip = "text")
  return(out3)}

has_discharge <- function(parameter){
  Q <- BULK_simplifyALL(parameter = "Discharge")
  Q$Location <- Q$LocationIdentifier
  Q$Q_units <- Q$Unit
  Q$Q_published <- Q$Publish
  Q$Q_length_of_record <- Q$Length_of_Obs
  Q$Q_start <- Q$Record_Start
  Q$Q_end <- Q$Record_End
  Q$Q_TSdata <- Q$Identifier
  
  P <- BULK_simplifyALL(parameter = parameter)
  P$Location <- P$LocationIdentifier
  P$P_units <- P$Unit
  P$P_published <- P$Publish
  P$P_length_of_record <- P$Length_of_Obs
  P$P_start <- P$Record_Start
  P$P_end <- P$Record_End
  P$P_TSdata <- P$Identifier
  
  QX <- Q[,c(11,12,13,14,15,16,17)]
  PX <- P[,c(11,12,13,14,15,16,17)]
  
  out <- merge(x= QX, y= PX, by= 'Location', all.x= F, all.y = F)
  return(out)}

#runs mann kendall test on a given record. best used with lapply to many datasets
INT_TS <- function(record){
  tsdata = timeseries$getTimeSeriesData(record)
  
  tsdataM <- TS_monthly(data = tsdata)
  require(Kendall)
  tsvector <- tsdataM$Mean
  MKtest <- MannKendall(tsvector)
  recordX <- record
  out <- (MKtest$sl)
  out2 <- as.data.frame(cbind(recordX, out))
  out2$record <- out2$recordX
  out2$MK_p <- out2$out
  out3 <- out2[,c("record","MK_p")]
